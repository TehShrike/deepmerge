...
test('merging objects with own __proto__ and null prototype', function(t) {
...
test('merging objects with own __proto__ with arguments reversed', function(t) {
	var user = {}
	var malicious = JSON.parse('{ "__proto__": { "admin": true } }')
	var mergedObject = merge(malicious, user)
	t.notOk(mergedObject.__proto__.admin, 'non-plain properties should not be merged')
	t.notOk(mergedObject.admin, 'the destination should have an unmodified prototype')
	t.end()
})

test('merging objects with own __proto__ with null prototype and arguments reversed', function(t) {
	var user = Object.create(null);
	var malicious = JSON.parse('{ "__proto__": { "admin": true } }')
	var mergedObject = merge(malicious, user)
	t.notOk(mergedObject.__proto__.admin, 'non-plain properties should not be merged')
...
test('merging objects with plain and non-plain properties', function(t) {
	...
	t.equal('foo', mergedObject.parentKey, 'inherited properties of target should be removed, not merged or ignored')
